...
for use within the HTML DOM, the important context/plan JSON object and
a `readyCallback` function, currently set in `edit.js` to rename the
window title.
...
Finally, `state.numbers` is not initialized here, but generated by
`NumberMarkers`.
