...
export default () => {
    listPlacesForState("California").then(places => {
        const target = document.getElementById("districting-options");
        render(districtingOptions(places), target);
        const target2 = document.getElementById("districting-options-2");
        render(districtingOptions(places.filter(p => p.id === "napaschools")), target2);
    });
