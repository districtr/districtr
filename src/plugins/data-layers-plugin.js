...
export default function DataLayersPlugin(editor) {
    ...
    if (state.problem.type !== "community") {
        ...
        let addSelectedCOIList = () => {
                ...
                cplan.plan.parts.forEach((part) => {
                    ...
                    layerkey.style.display = "block";
                      ...
                      ckbox.onchange = () => {
                          ...
                          let editLayer = (cplan.plan.units.id === "blockgroups") ? state.coiunits : state.coiunits2;
            ...
            if (hasCOI) {
                state.coiunits.setPaintProperty("fill-color", bgShades);
            } else {
                state.coiunits.setPaintProperty("fill-color", "rgba(0,0,0,0)");
            }
            if (hasCOI2) {
                state.coiunits2 && state.coiunits2.setPaintProperty("fill-color", preShades);
            } else {
                state.coiunits2 && state.coiunits2.setPaintProperty("fill-color", "rgba(0,0,0,0)");
            }
        ...
        >
            ...
            >
                ...
                <div style="text-align:center">
                  <button
                    style="border: 1px solid #000; padding:4px; user-select: none; border-radius: 4px; margin-top: 15px; margin-bottom: 15px"
                    @click=${e => addSelectedCOIList()}
                  >
                    Update COI List
                  </button>
        ...
        tab.addSection(() => html`
            ${toggle("View contributed COIs", true, checked => {
                let opacity = checked ? 0.8 : 0;
                state.coiunits.setOpacity(opacity);
                state.coiunits2 && state.coiunits2.setOpacity(opacity);
            ...
            <a
              ...
              @click="${showCOImodal}"
            >
              <img src="/assets/fullscreen.png" alt="expand" style="vertical-align:middle;width:20px;"/>
              Browse
            </a>
            <div id="coi_browse">
        ...
        const modurl = (window.location.hostname === "localhost")
                    ...
                    : (`/.netlify/functions/moduleRead?module=${state.place.id}&state=${state.place.state}&page=1`)
        ...
        fetch(modurl).then(res => res.json()).then((cois) => {
            ...
            coiMinder = new Tooltip(state.coiunits, tooltipVisFn, 0);
            coiMinder.activate();

            if (state.coiunits2) {
              coiMinder2 = new Tooltip(state.coiunits2, tooltipVisFn, 0);
              coiMinder2.activate();
            }
        });
    }
